...
public enum ReefPipe {
  ...
  public Pose2d getPose(ReefPipeLevel level, RobotScoringSide side) {
    if (side == RobotScoringSide.LEFT) {
      return FmsSubsystem.getInstance().isRedAlliance() ? redPoses.getLeftPose(level) : bluePoses.getLeftPose(level);
    }

    return FmsSubsystem.getInstance().isRedAlliance() ? redPoses.getRightPose(level) : bluePoses.getRightPose(level);
  }
