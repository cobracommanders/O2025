...
public class AlignmentCostUtil {
    ...
    public static double getAlignCost(
        ...
        var angleErrorCost =
                FeatureFlags.AUTO_ALIGN_HEADING_COST.getAsBoolean()
                        ? angleError * ANGLE_ERROR_SCALAR
                        : 0.0;
                ...
                FeatureFlags.AUTO_ALIGN_DRIVE_DIRECTION_COST.getAsBoolean()
                        ? DRIVE_DIRECTION_SCALAR
                                * Math.abs(
                                        targetDirection
                                                .minus(
                                                        MathHelpersDog.vectorDirection(
                                                                robotVelocity))
                                                .getRadians())
                        : 0.0;
    ...
    private final Comparator<ReefPipe> pipeL4Comparator =
            createReefPipeComparator(ReefPipeLevel.L4);
    private final Comparator<ReefPipe> pipeL3Comparator =
            createReefPipeComparator(ReefPipeLevel.L3);
    private final Comparator<ReefPipe> pipeL2Comparator =
            createReefPipeComparator(ReefPipeLevel.L2);
    private final Comparator<ReefPipe> pipeL1Comparator =
            createReefPipeComparator(ReefPipeLevel.L1);
    private final Comparator<ReefSide> algaeComparator = createAlgaeComparator();

    public AlignmentCostUtil(
            LocalizationSubsystem localization, DriveSubsystem swerve, RobotScoringSide side) {
        ...
        return switch (level) {
            case L1 -> {
                yield Comparator.comparingDouble(
                        pipe -> {
                            var allPipes =
                                    TagAlign.ALL_REEF_PIPES; // Assuming reefState has a method to
                            // get all pipes
