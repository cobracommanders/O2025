...
public class MechanismVisualizer {
    ...
    static MechanismLigament2d elevator =
            armRoot.append(new MechanismLigament2d("Elevator", 1.5, 90));
    static MechanismLigament2d arm =
            elevator.append(
                    new MechanismLigament2d("Arm", 0.7, 90, 6, new Color8Bit(Color.kPurple)));
    ...
    static MechanismLigament2d groundPivot =
            groundRoot.append(
                    new MechanismLigament2d(
                            "GroundPivot", 0.2, 90, 6, new Color8Bit(Color.kGreen)));
    ...
    public static void publishData() {
        ...
        if (Robot.isSimulation()) {
            ...
            if (CoralDetector.getInstance().hasCoral()) {
                Transform3d intakeCoralTransform =
                        switch (CoralDetector.getInstance().getState()) {
                            case LEFT ->
                                    new Transform3d(
                                            0.25, 0.1, 0.0, new Rotation3d(0.0, 0.0, Math.PI / 2));
                            case RIGHT ->
                                    new Transform3d(
                                            0.25, -0.1, 0.0, new Rotation3d(0.0, 0.0, Math.PI / 2));
                            default ->
                                    new Transform3d(
                                            0.25, 0.0, 0.0, new Rotation3d(0.0, 0.0, Math.PI / 2));
                        };

                DogLog.log(
                        "MechanismVisualizer/IntakeCoral",
                        new Pose3d[] {
                            robotPose3d.transformBy(
                                    new Transform3d(
                                            Pose3d.kZero,
                                            groundPivotPose.transformBy(intakeCoralTransform)))
                        });
            } else {
            ...
            // Display arm coral
            Transform3d armCoralTransform =
                    new Transform3d(0.35, -0.55, 0.0, new Rotation3d(0.0, Math.PI / 2, 0.0));
            Transform3d armAlgaeTransform =
                    new Transform3d(0.35, -0.7, 0.0, new Rotation3d(0.0, 0.0, 0.0));
            switch (Robot.armManager.getCurrentGamePiece()) {
                case CORAL -> {
                    DogLog.log(
                            "MechanismVisualizer/ArmCoral",
                            new Pose3d[] {
                                robotPose3d.transformBy(
                                        new Transform3d(
                                                Pose3d.kZero,
                                                armPose.transformBy(armCoralTransform)))
                            });
                    DogLog.log("MechanismVisualizer/ArmAlgae", new Pose3d[] {});
                }
                case ALGAE -> {
                    DogLog.log(
                            "MechanismVisualizer/ArmAlgae",
                            new Pose3d[] {
                                robotPose3d.transformBy(
                                        new Transform3d(
                                                Pose3d.kZero,
                                                armPose.transformBy(armAlgaeTransform)))
                            });
