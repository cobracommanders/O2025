...
public class ArmManager extends StateMachine<ArmManagerState> {
    ...
    private final Debouncer algaeDroppedOrMissingDebouncer =
            new Debouncer(0.3, Debouncer.DebounceType.kRising);

    public ArmManager(Hand hand, Elevator elevator, Arm arm) {
    ...
    public boolean isReadyToReturnToIdleAfterScoringCoral() {
        ...
        // TODO Is this just measuring distance from the target scoring pose? If so, would being
        // 0.25m to the left/right also allow the arm to move?
        return timeout(0.5)
    ...
    public boolean isReadyToReturnToIdleAfterIntakingAlgae() {
        return AutoAlign.getInstance()
                        .getAlgaeDistance()
                        .getTranslation()
                        .getDistance(LocalizationSubsystem.getInstance().getPose().getTranslation())
                >= 1.25;
    }
    ...
    protected ArmManagerState getNextState(ArmManagerState currentState) {
        ...
        switch (currentState) {
            ...
            /* ******** IDLE STATES ******** */
            case PREPARE_IDLE_EMPTY,
                    PREPARE_IDLE_CORAL_UP,
                    PREPARE_IDLE_CORAL_DOWN,
                    PREPARE_IDLE_ALGAE -> {
                if (atPosition()) nextState = currentState.getPrepareToIdleState();
            }

            case IDLE_ALGAE -> {
                if (algaeDroppedOrMissingDebouncer.calculate(!hand.droppedAlgae())
                        && !Robot.isSimulation()) {
            ...
            case IDLE_EMPTY -> {
                if (coralDetector.hasCoral()) {
                    nextState =
                            ArmManagerState.getHandoffPreemptiveFromCoralPosition(
                                    coralDetector.getState());
                }
            }

            case IDLE_CORAL_UP, IDLE_CORAL_DOWN -> {
                /* Await Control */
            }
            ...
            case PREEMPTIVE_HANDOFF_LEFT, PREEMPTIVE_HANDOFF_MIDDLE, PREEMPTIVE_HANDOFF_RIGHT -> {
                ...
                ArmManagerState targetPreemptiveState =
                        ArmManagerState.getHandoffPreemptiveFromCoralPosition(coralPosition);
            ...
            /* ******** ALGAE SCORE STATES ******** */
            case PREPARE_SCORE_ALGAE_NET_LEFT, PREPARE_SCORE_ALGAE_NET_RIGHT -> {
                if (atPosition()) {
            ...
            case SCORE_ALGAE_NET_LEFT, SCORE_ALGAE_NET_RIGHT, SCORE_ALGAE_PROCESSOR -> {
                if (timeout(1)) {
                    nextState = ArmManagerState.PREPARE_IDLE_EMPTY;
                }
            }
    ...
    protected void afterTransition(ArmManagerState newState) {
        switch (newState) {
            ...
            case PREPARE_IDLE_EMPTY, IDLE_EMPTY ->
                    requestState(
                            ArmState.HANDOFF_MIDDLE,
                            ElevatorState.PREPARE_HANDOFF,
                            HandState.IDLE_EMPTY);

            case PREPARE_IDLE_CORAL_UP, IDLE_CORAL_UP ->
                    requestState(
                            ArmState.IDLE_CORAL_UP,
                            ElevatorState.IDLE_CORAL_UP,
                            HandState.IDLE_CORAL);
            ...
            case IDLE_ALGAE_DROPPED ->
                    requestState(ArmState.IDLE_ALGAE, ElevatorState.IDLE, HandState.CLEAR_ALGAE);

            /* ******** HANDOFF STATES ******** */
            case PREPARE_HANDOFF_RIGHT,
                    PREEMPTIVE_HANDOFF_RIGHT,
                    PREPARE_HANDOFF_LEFT,
                    PREEMPTIVE_HANDOFF_LEFT,
                    PREPARE_HANDOFF_MIDDLE,
                    PREEMPTIVE_HANDOFF_MIDDLE ->
                    requestState(
                            ArmState.HANDOFF_MIDDLE,
                            ElevatorState.PREPARE_HANDOFF,
                            HandState.IDLE_EMPTY);

            case READY_HANDOFF_LEFT, READY_HANDOFF_RIGHT, READY_HANDOFF_MIDDLE ->
                    requestState(
                            ArmState.HANDOFF_MIDDLE,
                            ElevatorState.PREPARE_HANDOFF,
                            HandState.HANDOFF);

            case EXECUTE_HANDOFF_LEFT, EXECUTE_HANDOFF_RIGHT, EXECUTE_HANDOFF_MIDDLE ->
            ...
            case PREPARE_INVERTED_HANDOFF, READY_INVERTED_HANDOFF ->
                    requestState(
                            ArmState.HANDOFF_MIDDLE, ElevatorState.HANDOFF, HandState.IDLE_CORAL);
            case EXECUTE_INVERTED_HANDOFF ->
                    requestState(
                            ArmState.HANDOFF_MIDDLE,
                            ElevatorState.HANDOFF,
                            HandState.INVERTED_HANDOFF);

            /* ******** CORAL SCORE STATES ******** */
            case PREPARE_L4_LEFT, READY_L4_LEFT ->
                    requestState(
                            ArmState.PREPARE_L4_LEFT,
                            ElevatorState.PREPARE_L4,
                            HandState.IDLE_CORAL);
            case PREPARE_L3_LEFT, READY_L3_LEFT ->
                    requestState(
                            ArmState.PREPARE_L3_LEFT,
                            ElevatorState.PREPARE_L3,
                            HandState.IDLE_CORAL);
            case PREPARE_L2_LEFT, READY_L2_LEFT ->
                    requestState(
                            ArmState.PREPARE_L2_LEFT,
                            ElevatorState.PREPARE_L2,
                            HandState.IDLE_CORAL);
            case PREPARE_L4_RIGHT, READY_L4_RIGHT ->
                    requestState(
                            ArmState.PREPARE_L4_RIGHT,
                            ElevatorState.PREPARE_L4,
                            HandState.IDLE_CORAL);
            case PREPARE_L3_RIGHT, READY_L3_RIGHT ->
                    requestState(
                            ArmState.PREPARE_L3_RIGHT,
                            ElevatorState.PREPARE_L3,
                            HandState.IDLE_CORAL);
            case PREPARE_L2_RIGHT, READY_L2_RIGHT ->
                    requestState(
                            ArmState.PREPARE_L2_RIGHT,
                            ElevatorState.PREPARE_L2,
                            HandState.IDLE_CORAL);

            case SCORE_L4_LEFT ->
                    requestState(
                            ArmState.SCORE_L4_LEFT, ElevatorState.SCORE_L4, HandState.SCORE_CORAL);
            case SCORE_L3_LEFT ->
                    requestState(
                            ArmState.SCORE_L3_LEFT, ElevatorState.SCORE_L3, HandState.SCORE_CORAL);
            case SCORE_L2_LEFT ->
                    requestState(
                            ArmState.SCORE_L2_LEFT, ElevatorState.SCORE_L2, HandState.SCORE_CORAL);
            case SCORE_L4_RIGHT ->
                    requestState(
                            ArmState.SCORE_L4_RIGHT, ElevatorState.SCORE_L4, HandState.SCORE_CORAL);
            case SCORE_L3_RIGHT ->
                    requestState(
                            ArmState.SCORE_L3_RIGHT, ElevatorState.SCORE_L3, HandState.SCORE_CORAL);
            case SCORE_L2_RIGHT ->
                    requestState(
                            ArmState.SCORE_L2_RIGHT, ElevatorState.SCORE_L2, HandState.SCORE_CORAL);

            /* ******** ALGAE INTAKE STATES ******** */
            case PREPARE_INTAKE_HIGH_REEF_ALGAE_LEFT ->
                    requestState(
                            ArmState.INTAKE_HIGH_REEF_ALGAE_LEFT,
                            ElevatorState.HIGH_REEF_ALGAE,
                            HandState.INTAKE_REEF_ALGAE);
            case PREPARE_INTAKE_HIGH_REEF_ALGAE_RIGHT ->
                    requestState(
                            ArmState.INTAKE_HIGH_REEF_ALGAE_RIGHT,
                            ElevatorState.HIGH_REEF_ALGAE,
                            HandState.INTAKE_REEF_ALGAE);
            case PREPARE_INTAKE_LOW_REEF_ALGAE_LEFT ->
                    requestState(
                            ArmState.INTAKE_LOW_REEF_ALGAE_LEFT,
                            ElevatorState.LOW_REEF_ALGAE,
                            HandState.INTAKE_REEF_ALGAE);
            case PREPARE_INTAKE_LOW_REEF_ALGAE_RIGHT ->
                    requestState(
                            ArmState.INTAKE_LOW_REEF_ALGAE_RIGHT,
                            ElevatorState.LOW_REEF_ALGAE,
                            HandState.INTAKE_REEF_ALGAE);
            case PREPARE_INTAKE_GROUND_ALGAE ->
                    requestState(
                            ArmState.INTAKE_GROUND_ALGAE,
                            ElevatorState.GROUND_ALGAE,
                            HandState.INTAKE_GROUND_ALGAE);

            /* ******** ALGAE SCORE STATES ******** */
            case PREPARE_SCORE_ALGAE_NET_LEFT, READY_SCORE_ALGAE_NET_LEFT ->
                    requestState(
                            ArmState.ALGAE_NET_LEFT, ElevatorState.ALGAE_NET, HandState.IDLE_ALGAE);
            case PREPARE_SCORE_ALGAE_NET_RIGHT, READY_SCORE_ALGAE_NET_RIGHT ->
                    requestState(
                            ArmState.ALGAE_NET_RIGHT,
                            ElevatorState.ALGAE_NET,
                            HandState.IDLE_ALGAE);
            case PREPARE_SCORE_ALGAE_PROCESSOR, READY_SCORE_ALGAE_PROCESSOR ->
                    requestState(
                            ArmState.ALGAE_PROCESSOR,
            ...
            case SCORE_ALGAE_PROCESSOR ->
                    requestState(
                            ArmState.ALGAE_PROCESSOR,
                            ElevatorState.ALGAE_PROCESSOR,
                            HandState.SCORE_ALGAE_PROCESSOR);

            /* ******** LOLLIPOP INTAKE STATES ******** */
            case PREPARE_INTAKE_LOLLIPOP ->
                    requestState(
                            ArmState.PREPARE_LOLLIPOP, ElevatorState.LOLLIPOP, HandState.LOLLIPOP);
            case ACTIVE_INTAKE_LOLLIPOP ->
                    requestState(ArmState.LOLLIPOP, ElevatorState.LOLLIPOP, HandState.LOLLIPOP);

            /* ******** CLIMB STATES ******** */
            case PREPARE_CLIMB, READY_CLIMB ->
                    requestState(ArmState.CLIMB, ElevatorState.IDLE, HandState.CLEAR_ALGAE);
        }
    }
    ...
    public void requestHandoff() {
        if (getState().handGamePieceState.isNone()) {
            setStateFromRequest(
                    ArmManagerState.getHandoffPrepareFromCoralPosition(coralDetector.getState()));
        }
    }
    ...
    public void requestIdle() {
        ArmManagerState state =
                switch (getState().handGamePieceState) {
                    case CORAL ->
                            armScheduler.isArmUp()
                                    ? ArmManagerState.IDLE_CORAL_UP
                                    : ArmManagerState.IDLE_CORAL_DOWN;
    ...
    public void requestCoralPrepare(
            RobotScoringSide robotSide, FieldConstants.PipeScoringLevel scoringLevel) {
        ArmManagerState coralPrepareState =
                ArmManagerState.getCoralPrepareScore(robotSide, scoringLevel);
        if (getState().handGamePieceState.isCoral()
                && getState() != coralPrepareState.getCoralPrepareToReadyState()) {
    ...
    public static class CommandWrapper {
        ...
        public Command requestAlgaeProcessorPrepareAndAwaitReady() {
            return armManager
                    .runOnce(armManager::requestProcessorScore)
        ...
        public Command executeAlgaeProcessorScoreAndAwaitIdle() {
            return armManager
                    .runOnce(armManager::requestProcessorScoreExecution)
                    .andThen(armManager.waitForState(ArmManagerState.PREPARE_IDLE_EMPTY))
                    .withName("executeAlgaeProcessorScoreAndAwaitIdle");
        }

        /**
         * Request algae net score and await ready state. Will ONLY use the scoring side from the
         * supplier at the time of scheduling.
         */
        public Command requestAlgaeNetPrepareAndAwaitReady(Supplier<RobotScoringSide> side) {
            return armManager
                    .runOnce(() -> armManager.requestAlgaeNetScore(side.get()))
        ...
        public Command executeAlgaeNetScoreAndAwaitIdle() {
            return armManager
                    .runOnce(armManager::requestAlgaeNetScoreExecution)
                    .andThen(armManager.waitForState(ArmManagerState.PREPARE_IDLE_EMPTY))
        ...
        public Command requestClimbAndDoNothing() {
            return armManager
                    .runOnce(armManager::requestClimb)
                    .andThen(doNothing())
                    .withName("requestClimbAndDoNothing");
        }

        /**
         * Prepare for coral score and await ready state. Will constantly update the scoring side
         * and level based on the value of the supplier.
         */
        public Command requestCoralPrepareAndAwaitReady(
                Supplier<RobotScoringSide> side,
                Supplier<FieldConstants.PipeScoringLevel> scoringLevel) {
            return armManager
                    .runOnce(() -> armManager.requestCoralPrepare(side.get(), scoringLevel.get()))
                    .repeatedly()
                    .until(armManager::isReadyToScoreCoral)
        ...
        public Command executeCoralScoreAndAwaitComplete() {
            return armManager
                    .runOnce(armManager::requestCoralScoreExecution)
        ...
        public Command idleClearGamePieceAndAwaitReady() {
            return armManager
                    .runOnce(armManager::requestIdleClearGamePiece)
                    .andThen(Commands.waitUntil(armManager::isIdleState))
                    .withName("requestIdleClearGamePieceAndAwaitReady");
        }

        /**
         * Request algae reef intake and await idle state after collecting a game piece. Will
         * constantly update the scoring side based on the value of the supplier.
         */
        public Command requestAlgaeReefIntakeAndAwaitIdle(
                Supplier<RobotScoringSide> side, boolean top) {
            return armManager
                    .runOnce(() -> armManager.requestReefAlgaeIntake(side.get(), top))
                    .repeatedly()
        ...
        public Command requestGroundAlgaeIntakeAndAwaitGamePiece() {
            return armManager
                    .runOnce(armManager::requestGroundAlgaeIntake)
        ...
        public Command executeHandoffUntilIntakeWait() {
            return armManager
                    .runOnce(armManager::requestHandoffExecution)
        ...
        public Command executeInvertedHandoff() {
            return armManager
                    .runOnce(armManager::requestInvertedHandoffExecution)
