...
public class ArmScheduler extends StateMachine<ArmSchedulerState> {
    ...
    public ArmScheduler(Arm arm, Elevator elevator, Hand hand) {
        super(ArmSchedulerState.READY);
        this.arm = arm;
        this.elevator = elevator;
        this.hand = hand;
        this.visualization =
                new ArmSchedulerVisualization(
                        driveWidth, driveHeight, intakeWidth, finalIntakeHeight);
    }

    private final double armLength = Units.inchesToMeters(24.5);
    private final double armWidth =
            Units.inchesToMeters(5.0); // Left-right width of the arm when viewed from the front
    private final double handHeight =
            Units.inchesToMeters(
                    5.0); // Used for algae floor intake, the length of arm that can contact the
    // intake when the arm is horizontal
    ...
    private final double finalIntakeHeight =
            intakePivotHeight + intakeMinInterferenceUpHeightFromPivot + intakeHeightOffset;
    private final double intakeWidth = Units.inchesToMeters(21.25);

    private ArmSchedulerState assessState() {
        boolean canMoveElevatorInternally =
                !willArmHitIntakeOrDrivetrain(
                        arm.getNormalizedPosition(), targetElevatorState.getPosition());
        boolean canMoveArmInternally =
                !willArmHitIntakeOrDrivetrain(targetArmState.getPosition(), elevator.getHeight());

        // Ideally no longer needed with new coral autoalign

        //        boolean willArmExtendOutOfFrame =
        // willArmExtendOutOfFrame(targetArmState.getPosition());
        //        boolean isArmExtendingOutOfFrame =
        // willArmExtendOutOfFrame(arm.getNormalizedPosition());
    ...
    private boolean willArmHitIntakeOrDrivetrain(double armPosition, double elevatorPosition) {
        return willArmHitIntake(armPosition, elevatorPosition)
                || willArmHitDrivetrain(armPosition, elevatorPosition);
    }
    ...
    private boolean willArmHitIntake(double armPosition, double elevatorPosition) {
        ...
        return (coordinate1.y() < finalIntakeHeight
                        && abs(coordinate1.x() - intakeWidth) > handHeight)
                || (coordinate2.y() < finalIntakeHeight
                        && abs(coordinate2.x() - intakeWidth) > handHeight);
    }
    ...
    private boolean willArmHitDrivetrain(double armPosition, double elevatorPosition) {
        ...
        Coordinate coordinate1 = coordinates[0];
        Coordinate coordinate2 = coordinates[1];
        return (coordinate1.y() < driveHeight && abs(coordinate1.x() - driveWidth) > handHeight)
                || (coordinate2.y() < driveHeight
                        && abs(coordinate2.x() - driveWidth) > handHeight);
    }
    ...
    /**
     * Checks if the arm will swing through the outside of the frame perimiter, but starts and ends
     * within the frame.
     */
    private boolean willArmSwingThroughOutsideFrame(double armPosition, double targetArmPosition) {
        return !willArmExtendOutOfFrame(armPosition)
                && !willArmExtendOutOfFrame(targetArmPosition)
                && (isArmUp(armPosition) != isArmUp(targetArmPosition));
    }
    ...
    private Coordinate[] getArmCoordinates(double armPosition, double elevatorPosition) {
        ...
        var armEndCenterY =
                (Math.sin(armPositionRadians) * armLength) + elevatorBaseHeight + elevatorPosition;
    ...
    /**
     * Gets the arm state that ensures that if the arm will swing through the outside of the frame
     * perimiter, it will swing in the correct direction away from the reef.
     */
    private ArmState getArmStateWithSwingDirection(ArmState state) {
        boolean willSwingOutOfFrame =
                willArmSwingThroughOutsideFrame(arm.getNormalizedPosition(), state.getPosition());
        RobotScoringSide reefSide =
        ...
        if (willSwingOutOfFrame) {
            // RobotScoringSide reefSide =
            // AutoAlign.getScoringSideFromRobotPose(LocalizationSubsystem.getInstance().getPose());
            //    return switch (reefSide) {
            //        case RIGHT -> ArmState.TRANSITION_OUTSIDE_FRAME_LEFT;
            //        case LEFT -> ArmState.TRANSITION_OUTSIDE_FRAME_RIGHT;
            //    };
    ...
    public void periodic() {
        ...
        switch (getState()) {
            ...
            case ELEVATOR_FIRST -> {
                elevator.setState(targetElevatorState);
                double armPosition =
                        getNearestArmPositionWithoutCollision(
                                elevator.getHeight(),
                                targetArmState.getPosition(),
                                arm.getNormalizedPosition());
                arm.setCustom(armPosition);
            }
            case ARM_FIRST -> {
                arm.setState(getArmStateWithSwingDirection(targetArmState));
                double elevatorPosition =
                        getNearestElevatorHeightWithoutArmCollision(
                                arm.getNormalizedPosition(), targetElevatorState.getPosition());
    ...
    /**
     * Gets the elevator position that will be closest to the desired position while not hitting the
     * intake or drivetrain with the arm.
     */
    public double getNearestElevatorHeightWithoutArmCollision(
            double armPosition, double targetElevatorPosition) {
        // This method will only be called in ELEVATOR_FIRST mode, so we know the arm would hit the
        // intake or drivetrain if the elevator moved all the way, therefore move just above the
        // intake to be safe
        ...
        if (useIntakeHeight == true) {
            ...
            double minElevatorPositionForArm =
                    getElevatorPositionForDesiredArmHeight(armPosition, desiredHeight);
            ...
            double minElevatorPositionForArmWithoutIntake =
                    getElevatorPositionForDesiredArmHeight(armPosition, desiredHeightWithoutIntake);
    ...
    /**
     * Gets the arm position that will be closest to the desired position while not hitting the
     * intake or drivetrain.
     */
    public double getNearestArmPositionWithoutCollision(
            double elevatorPosition, double targetArmPosition, double armPosition) {
        // This method will only be called in ARM_FIRST mode, so we know the arm would hit the
        // intake or drivetrain if it moved all the way, therefore move just above the intake to be
        // safe
        double desiredHeight = finalIntakeHeight + (armWidth / 2.0) + Units.inchesToMeters(1.0);

        double solutionOne =
                Arm.normalizePosition(
                        getArmPositionForDesiredArmHeight(elevatorPosition, desiredHeight));
    ...
    /**
     * Gets the elevator position that will put the center of the end of the arm at the desired
     * height from the floor.
     */
    public double getElevatorPositionForDesiredArmHeight(
            double armPosition, double desiredArmHeight) {
    ...
    /**
     * Gets an arm position that will put the center of the end of the arm at the desired height
     * from the floor.
     */
    public double getArmPositionForDesiredArmHeight(
            double elevatorHeight, double desiredArmHeight) {
    ...
    protected void collectInputs() {
        ...
        DogLog.log(
                "ArmScheduler/armHittingIntake",
                willArmHitIntake(arm.getNormalizedPosition(), elevator.getHeight()));
        DogLog.log(
                "ArmScheduler/armHittingDrivetrain",
                willArmHitDrivetrain(arm.getNormalizedPosition(), elevator.getHeight()));
        DogLog.log(
                "ArmScheduler/armExtendingOutOfFrame",
                willArmExtendOutOfFrame(arm.getNormalizedPosition()));
        Coordinate[] coordinates =
                getArmCoordinates(arm.getNormalizedPosition(), elevator.getHeight());
    ...
    public void scheduleStates(
            ArmState armState, ElevatorState elevatorState, HandState handState) {
