...
public class IntakePivot extends StateMachine<IntakePivotStates> {
    ...
    public final TalonFXConfiguration motor_config =
            new TalonFXConfiguration()
                    .withSlot0(
                            new Slot0Configs()
                                    .withKP(IntakePivotConstants.P)
                                    .withKI(IntakePivotConstants.I)
                                    .withKD(IntakePivotConstants.D)
                                    .withKG(IntakePivotConstants.G)
                                    .withGravityType(
                                            GravityTypeValue
                                                    .Arm_Cosine)) // the intake doesn't need to use
    ...
    private IntakePivot() {
        ...
        encoder =
                new DutyCycle(
                        new DigitalInput(Ports.IntakePivotPorts.INTAKE_PIVOT_DUTY_CYCLE_ENCODER));
        ...
        motor_config.MotionMagic.MotionMagicCruiseVelocity =
                IntakePivotConstants.MotionMagicCruiseVelocity;
        motor_config.MotionMagic.MotionMagicAcceleration =
                IntakePivotConstants.MotionMagicAcceleration;
    ...
    public boolean atGoal() {
        return switch (getState()) {
            ...
            case INTAKING ->
                    MathUtil.isNear(IntakePivotPositions.INTAKING, intakePosition, tolerance);
            case HANDOFF ->
                    MathUtil.isNear(IntakePivotPositions.HANDOFF, intakePosition, tolerance);
            case SCORE_L1 ->
                    MathUtil.isNear(IntakePivotPositions.SCORE_L1, intakePosition, tolerance);
    ...
    private void updateSimPosition(double position) {
        ...
        talonSim.setRawRotorPosition(
                pivotSim.getAngularPosition().times(IntakePivotConstants.PivotGearRatio));
        talonSim.setRotorVelocity(
                pivotSim.getAngularVelocity().times(IntakePivotConstants.PivotGearRatio));
    }
    ...
    public static IntakePivot getInstance() {
        if (instance == null) instance = new IntakePivot();
        return instance;
    }
}
