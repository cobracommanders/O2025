...
public class Trailblazer {
                ...
                Commands.parallel(
                                ...
                                Commands.run(
                                        () -> {
                                            ...
                                            var currentAutoPointIndex =
                                                    pathTracker.getCurrentPointIndex();
                                            var currentAutoPoint =
                                                    segment.points.get(currentAutoPointIndex);
                                            double distanceToSegmentEnd =
                                                    segment.getRemainingDistance(
                                                            localization.getPose(),
                                                            currentAutoPointIndex);
                                            ...
                                            swerve.setFieldRelativeAutoSpeeds(
                                                    constrainedVelocityGoal);

                                            DogLog.log(
                                                    "Autos/Trailblazer/Tracker/CurrentPointIndex",
                                                    currentAutoPointIndex);
                                            if (previousAutoPointIndex != currentAutoPointIndex) {
                                                // Currently tracked point has changed, trigger side
                                                // effects
    ...
    private ChassisSpeeds getSwerveSetpoint(
        ...
        var rawVelocityGoal =
                new TimestampedChassisSpeeds(pathFollower.calculateSpeeds(currentPose, targetPose));

        // Get point-specific constraints if applicable, otherwise use the constraints of the full
        // segment
        ...
        var constrainedVelocityGoal =
                AutoConstraintCalculator.constrainVelocityGoal(
                        rawVelocityGoal, previousSpeeds, constraints, distanceToSegmentEnd);

        // Update previous speeds for acceleration calculations
        previousSpeeds = constrainedVelocityGoal;

        DogLog.log(
                "Autos/Trailblazer/Constraints/Linear Velocity", constraints.maxLinearVelocity());
        DogLog.log(
                "Autos/Trailblazer/Constraints/Linear Acceleration",
                constraints.maxLinearAcceleration());

        DogLog.log(
                "Autos/Trailblazer/Constraints/Angular Velocity", constraints.maxAngularVelocity());
        DogLog.log(
                "Autos/Trailblazer/Constraints/Angular Acceleration",
